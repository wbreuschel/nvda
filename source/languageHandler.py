...
From the locale.windows_locale in-line code documentation:
...
def windowsLCIDToLocaleName(lcid: int) -> Optional[str]:
	...
	# #4203: some locale identifiers from Windows 7+ do not exist in locale.windows_locale
	# Check a manual mapping before using Windows to look up the LCID locale name
	if not localeName:
